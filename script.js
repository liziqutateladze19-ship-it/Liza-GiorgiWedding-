/* ==========================================================
   Geo Rustaveli / Geo Tabidze არის Latin-QWERTY encoded ფონტები.
   ეს helper ხილულ ქართულ dynamic ტექსტებს შესაბამის glyph mapping-ზე გადაიყვანს.
   ========================================================== */
const LEGACY_GEO_MAP = {
...
function toLegacyGeorgian(text) {
  return Array.from(String(text))
    .map((ch) => LEGACY_GEO_MAP[ch] || ch)
    .join("");
}
...
const CONFIG = {
  ...
  // Google Sheets RSVP:
  // აქ ჩასვი Google Apps Script Web App-ის /exec URL.
  // თუ ცარიელია და საიტი Netlify-ზეა, ძველი Netlify Forms fallback იმუშავებს.
  rsvpEndpoint: ""
};
...
rsvpForm.addEventListener("submit", async (event) => {
  ...
  if (validationError) {
    formError.textContent = toLegacyGeorgian(validationError);
    return;
  }
  ...
  submitButton.textContent = toLegacyGeorgian("იგზავნება...");
  ...
  try {
    ...
    if (endpoint) {
      ...
      // no-cors საჭიროა Apps Script-ის redirect-ის გამო.
      ...
      // Netlify fallback — სანამ ძველი ვერსია ჯერ კიდევ გამოიყენება.
    ...
    console.error(error);
    formError.textContent = toLegacyGeorgian("პასუხის გაგზავნა ვერ მოხერხდა. გთხოვთ, სცადოთ თავიდან.");
  } finally {
    submitButton.disabled = false;
    submitButton.textContent = toLegacyGeorgian("გაგზავნა");
  }
});

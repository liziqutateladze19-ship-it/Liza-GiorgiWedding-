...
/* ==========================================================
   ...
   - audio ფაილი უნდა იდოს repository-ის root-ში.
   - კოდი ცდილობს დაკვრას გვერდის ჩატვირთვისთანავე და ყველა შესაძლო resume event-ზე.
   - თუ iPhone/Safari/Chrome ხმიან autoplay-ს სისტემურად ბლოკავს,
     პირველივე touch / scroll gesture-ზე მუსიკა ავტომატურად დაიწყება.
   ========================================================== */
...
async function startMusicIfPossible() {
  if (!backgroundMusic || !musicEnabled) return false;
...
function aggressivelyStartMusic() {
  if (!backgroundMusic || !musicEnabled || musicStarted) return;
  backgroundMusic.load();
  startMusicIfPossible();
}

// ვცდით რაც შეიძლება ადრე.
document.addEventListener("DOMContentLoaded", aggressivelyStartMusic, { once: true });
window.addEventListener("load", aggressivelyStartMusic, { once: true });
window.addEventListener("pageshow", aggressivelyStartMusic);
window.addEventListener("focus", startMusicIfPossible);
backgroundMusic?.addEventListener("canplay", startMusicIfPossible);
backgroundMusic?.addEventListener("loadeddata", startMusicIfPossible);

document.addEventListener("visibilitychange", () => {
  if (!document.hidden) startMusicIfPossible();
});
...
// თუ ბრაუზერმა initial autoplay დაბლოკა, სტუმრის პირველი შეხება/სქროლის gesture ააქტიურებს ხმას.
["pointerdown", "touchstart", "mousedown", "keydown"].forEach((eventName) => {
  window.addEventListener(eventName, startMusicIfPossible, { capture: true, passive: true });
});
